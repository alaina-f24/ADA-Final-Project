...
cerv <- cerv %>%
  mutate(cause_spec_death = as.factor(cause_spec_death), 
         ...
         sex = as.factor(sex),
         age = as.numeric(gsub("years", "", age)), ##maybe try using parse_number() function instead?
...
##factor vstat
#cerv$vstat <- factor(cerv$vstat, levels=c(0:1), labels=c("Alive","Dead"))
...
label(cerv_3$income_cat) <- "Income"
...
label(cerv_3$rural_urban) <- "Rural-Urban Residence"
label(cerv_3$cause_spec_death) <- "Cause of Death"
label(cerv_3$year_dx) <- "Year of Diagnosis"

##initial table
table1( ~ age + surv_months + vstat + cause_spec_death + year_dx + rural_urban + income_cat| race_eth, overall = "Total", rowlabelhead = "Variable", footnote = "SD = standard deviation", cerv_3)
...
**NH American Indian/Alaska Native has non-significant difference from NH White.
...
##Retry Adjusted Cox PH Regression model with year_cat variable (linearity violated for year_dx)
```{r}
...
##Try RMST analysis instead (due to PH assumption violation) 
```{r}
...
-  The differences in RMST between NH Black vs. NH White was -4.2 months, NH Asian vs. NH White was 1.6 momths, NH American Indian vs NH White was 0.9 months, and Hispanic vs. NH White was 1.4 months, respectively. 
